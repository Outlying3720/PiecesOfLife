...
class BaseDataset(Dataset):
    ...
    def __init__(self,
        ...
        n = 4
        jobs = list(range(len(self)))
        chunk_size = len(jobs) // n
        # part_jobs = []
        for i in range(0, len(jobs), chunk_size):
            part_job = jobs[i:i+chunk_size]
            # part_jobs.append(jobs[i:i+chunk_size])
            thread = Thread(target=BaseDataset.preload_data, args=(self, part_job))
            thread.start()
    ...
    def preload_data(self, jobs: List):
        print('#############preload_data')
        global queue
        try:
            for idx in jobs:
...
class BackgroundDataset(Thread):
    def __init__(self, basedataloader, max_prefetch=1, preprocess_func=None):
        """
        Parameters
        ----------
        ...
        self.basedataloader  = basedataloader
        self.length          = len(basedataloader.data_list)
    ...
    def run(self):
        try:
            for idx in range(self.length):  self.queue.put((True , self.basedataloader[idx]))
...
#     def __init__(self,
#                  *args, **kwargs):
#         basedatatset = _BaseDataset(*args, **kwargs)
#         return BackgroundDataset(basedatatset, 8)