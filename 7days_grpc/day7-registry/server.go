...
func (server *Server) handleRequest(cc codec.Codec, req *request, sending *sync.Mutex, wg *sync.WaitGroup, timeout time.Duration) {
	...
	case <-time.After(timeout):
		req.h.Error = fmt.Sprintf("rpc server: handle request timeout after %s", timeout)
		server.sendResponse(cc, req.h, invalidRequest, sending)
	case <-called:
		<-sent
	}
...
func (server *Server) findService(serviceMethod string) (svc *service, mtype *methodType, err error) {
	...
	if dot < 0 {
		err = errors.New("rpc server: bad service request: " + serviceMethod)
		return
	}
...
func (server *Server) HandleHTTP() {
	...
	log.Println("rpc: server start at:", defaultRPCPath)
	log.Println("rpc: debug server start at:", defaultDebugPath)
}
